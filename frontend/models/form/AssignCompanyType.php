...
use frontend\models\TCompanyCategory;
...
class AssignCompanyType extends Model
{
    ...
    public function addChildren($id,$items)
    {
        ...
        foreach ($items as $name) {
            try {
    					$companycateg = new TCompanyCategory();
    					$companycateg->t_vendor_company_id = $id;
    					$companycateg->p_master_category_id = $name[$i];
    					$companycateg->create_user = Yii::$app->user->identity->id;
    					$companycateg->update_user = Yii::$app->user->identity->id;
				      $companycateg->save();
			        $i++;
            } catch (\Exception $exc) {
    ...
    public function removeChildren($id,$items)
    {
        ...
        foreach ($items as $name) {
            try {
					      TCompanyCategory::deleteAll('t_vendor_company_id = '.$id.' AND p_master_category_id = '.$name[$i]);
                $i++;
            } catch (\Exception $exc) {
                Yii::error($exc->getMessage(), __METHOD__);
            }
