...
/**
 * BarangController implements the CRUD actions for TVendorTeknisKomoditi model.
 */
class BarangController extends Controller
{
    ...
    {
        $searchModel = new TVendorTeknisKomoditiBarangSearch();
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $model->merk = $_POST['TVendorTeknisKomoditi']['merk'];
            $model->sumber = $_POST['TVendorTeknisKomoditi']['sumber'];
            $model->p_master_pemasok_id = $_POST['TVendorTeknisKomoditi']['p_master_pemasok_id'];
            $model->area = $_POST['TVendorTeknisKomoditi']['area'];
            $model->unit = $_POST['TVendorTeknisKomoditi']['unit'];
            $model->jenis = 1;
            $model->uploadFile = UploadedFile::getInstance($model, 'uploadFile');
            $model->lampiran = preg_replace('/[^A-Za-z0-9\-]/', '_', Yii::$app->user->identity->id.'_gambar_barang_'.date('d m y h:i:s')) . '.' . $model->uploadFile->extension;
            $model->create_user = Yii::$app->user->identity->id;
            $model->update_user = Yii::$app->user->identity->id;
            $model->save();
            $model->uploadFile->saveAs($path . preg_replace('/[^A-Za-z0-9\-]/', '_',  Yii::$app->user->identity->id.'_gambar_barang_'.date('d m y h:i:s')) . '.' . $model->uploadFile->extension);
   ...
   public function actionDeleteHarga($id,$hid)
   {
       ...
       $harga = $this->findModelHarga($hid);
       $harga->masih_berlaku = false;
       $harga->is_delete = true;
       $harga->update();
